...
namespace OsuHook.Patches
{
    ...
    public class PatchFixNoGroupSorting : BasePatch
    {
        ...
        public static int GetSortMethodForGrouping(
            ...
            int newSortMode)
        {
            var beatmapTreeManager = SongSelection.beatmapTreeManager.GetValue(@this);
            var currentGroupModeBindable = BeatmapTreeManager.CurrentGroupMode.GetValue(beatmapTreeManager);
            var currentGroupMode = (int)Bindable.Value_get.Invoke(currentGroupModeBindable, new object[0]);
